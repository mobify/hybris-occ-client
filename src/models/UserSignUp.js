/* * *  *  * *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  * */
/* Copyright (c) 2017 Mobify Research & Development Inc. All rights reserved. */
/* * *  *  * *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  * */

/**
 * OCC No description provided (generated by Swagger Codegen
 * https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v2
 *
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git Do not edit the class
 * manually.
 *
 */

import ApiClient from '../ApiClient'

/**
* The UserSignUp model module.
* @module models/UserSignUp
* @version v2
*/
export default class UserSignUp {
    /**
    * Constructs a new <code>UserSignUp</code>.
    * @alias module:models/UserSignUp
    * @class
    */

    constructor() {
        /**
    *
    * @member {String} uid
    */
        this.uid = undefined
        /**
    *
    * @member {String} firstName
    */
        this.firstName = undefined
        /**
    *
    * @member {String} lastName
    */
        this.lastName = undefined
        /**
    *
    * @member {String} titleCode
    */
        this.titleCode = undefined
        /**
    *
    * @member {String} password
    */
        this.password = undefined
    }

    /**
    * Constructs a <code>UserSignUp</code> from a plain JavaScript object,
    * optionally creating a new instance. Copies all relevant properties from
    * <code>data</code> to <code>obj</code> if supplied or a new instance if
    * not.
    * @param {Object} data The plain JavaScript object bearing properties of
    * interest.
    * @param {module:models/UserSignUp} obj Optional instance to populate.
    * @return {module:models/UserSignUp} The populated
    * <code>UserSignUp</code> instance.
    */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new UserSignUp()

            if (data.hasOwnProperty('uid')) {
                obj.uid = ApiClient.convertToType(data.uid, 'String')
            }
            if (data.hasOwnProperty('firstName')) {
                obj.firstName = ApiClient.convertToType(data.firstName, 'String')
            }
            if (data.hasOwnProperty('lastName')) {
                obj.lastName = ApiClient.convertToType(data.lastName, 'String')
            }
            if (data.hasOwnProperty('titleCode')) {
                obj.titleCode = ApiClient.convertToType(data.titleCode, 'String')
            }
            if (data.hasOwnProperty('password')) {
                obj.password = ApiClient.convertToType(data.password, 'String')
            }
        }
        return obj
    }



}
