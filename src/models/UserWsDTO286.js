/**
 * OCC
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v2
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import Currency from './Currency';
import Language from './Language';

/**
* The UserWsDTO286 model module.
* @module models/UserWsDTO286
* @version v2
*/
export default class UserWsDTO286 {
    /**
    * Constructs a new <code>UserWsDTO286</code>.
    * @alias module:models/UserWsDTO286
    * @class
    */

    constructor() {
        
    }

    /**
    * Constructs a <code>UserWsDTO286</code> from a plain JavaScript object, optionally creating a new instance.
    * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
    * @param {Object} data The plain JavaScript object bearing properties of interest.
    * @param {module:models/UserWsDTO286} obj Optional instance to populate.
    * @return {module:models/UserWsDTO286} The populated <code>UserWsDTO286</code> instance.
    */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new UserWsDTO286();

            if (data.hasOwnProperty('titleCode')) {
                obj['titleCode'] = ApiClient.convertToType(data['titleCode'], 'String');
            }
            if (data.hasOwnProperty('firstName')) {
                obj['firstName'] = ApiClient.convertToType(data['firstName'], 'String');
            }
            if (data.hasOwnProperty('lastName')) {
                obj['lastName'] = ApiClient.convertToType(data['lastName'], 'String');
            }
            if (data.hasOwnProperty('currency')) {
                obj['currency'] = Currency.constructFromObject(data['currency']);
            }
            if (data.hasOwnProperty('language')) {
                obj['language'] = Language.constructFromObject(data['language']);
            }
        }
        return obj;
    }

    /**
    * 
    * @member {String} titleCode
    */
    titleCode = undefined;
    /**
    * 
    * @member {String} firstName
    */
    firstName = undefined;
    /**
    * 
    * @member {String} lastName
    */
    lastName = undefined;
    /**
    * @member {module:models/Currency} currency
    */
    currency = undefined;
    /**
    * @member {module:models/Language} language
    */
    language = undefined;

}
